    ...
    var body: some View {
        HStack {
            if message.messageType == .call {
                CallMessageView(message: message, isCurrentUser: isCurrentUser)
            } else {
                HStack(spacing: 2) {
                    if !isCurrentUser {
                        VStack {
...
struct CallMessageView: View {
    ...
    var body: some View {
        HStack {
            ...
            HStack {
                ...
                Image(systemName: message.callInfo?.callType == "video" ? "video.fill" : "phone.fill")
                    .foregroundColor(isCurrentUser ? .white : rgbToColor(red: 27, green: 26, blue: 87))

                HStack {
                    Text(getCallStatusText())
                        .foregroundColor(isCurrentUser ? .white : rgbToColor(red: 27, green: 26, blue: 87))
                    if let duration = message.callInfo?.duration {
    ...
    private func getCallStatusText() -> String {
        let direction = isCurrentUser ? "Outgoing" : "Incoming"
        let type = message.callInfo?.callType == "video" ? "Video" : "Voice"
        let status = message.callInfo?.status == "rejected" ? "Missed" : "call"
